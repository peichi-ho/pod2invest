...
- arguments 切法（topic 只能用以下三種類型的命名方式，不可自創其他分類名稱）：
  【固定結構型】以下三個主題若逐字稿有實質內容就必須出現，沒有才略過：
    ...
    ・不涉及特定標的的交易心法、資產配置、市場生存法則等策略性內容，一律併入「操作策略與建議」
    不要為這類內容另外發明「金融市場」「投資觀點：xxx」這種新標題。
  【個股型】逐字稿中被認真討論（3 句以上）的每一支股票／ETF／指數，
    各自獨立一條，topic 命名為「個股：[名稱]」，例如「個股：台積電」
    不論節目怎麼包裝這段內容（公司分析、個股觀察、財報解讀…），只要是針對單一公司/標的的討論，
    一律用「個股：[名稱]」，不可用其他前綴或自訂標題。
  【產業型】逐字稿中被認真討論（3 句以上）的每一個產業主題，
    各自獨立一條，topic 統一命名為「產業：[主題名稱]」，例如「產業：記憶體漲價」「產業：AI伺服器需求」「產業：PCB族群」
  【完整性要求】動筆前先掃描全文，列出所有被認真討論（3句以上）、且屬於上述三種類型的主題清單，
    再逐一確認每個主題都有對應的 argument，不可因為想省事就遺漏
  【排除規則】逐字稿中若有跟投資／市場／公司／總體經濟完全無關的內容
    （例如主持人的個人生活、育兒經驗、興趣嗜好、名人八卦閒聊等），不要為此建立 argument，直接略過，
    這不算違反上面的完整性要求
