...
REQUIRED_PY_PKGS = ["requests", "feedparser", "openai", "google-generativeai", "python-dotenv"]
...
def input_int(prompt: str, default: int, min_value: int = 0) -> int:
    ...
    try:
        v = int(s)
        if v < min_value: raise ValueError
        return v
    except ValueError:
...
def itunes_search_first_podcast(term: str, limit: int = 5) -> dict[str, Any]:
    ...
    if not results: raise RuntimeError(f"查無結果：{term}")
    return results[0]

def extract_enclosure_url(entry) -> str:
    """從 RSS 項目中提取音檔下載連結"""
    if hasattr(entry, "enclosures") and entry.enclosures:
        href = getattr(entry.enclosures[0], "href", None) or entry.enclosures[0].get("href")
        if href: return href
    if hasattr(entry, "links") and entry.links:
        for link in entry.links:
            if link.get("rel") == "enclosure": return link["href"]
...
def transcribe_and_fix(audio_path: Path, out_path: Path, language: str | None, gemini_key: str):
    """
    轉錄流程管理：
    ...
    print(f"   [Step 2] 轉錄完成（共 {len(srt_blocks)} 區塊），啟動 Gemini AI 語意校對...")
    for i in range(0, len(srt_blocks), batch_size):
        batch = "\n\n".join(srt_blocks[i : i + batch_size])
        print(f"      處理進度：校正第 {i+1} ~ {min(i + batch_size, len(srt_blocks))} 句...")
...
def main():
    ...
    project_root = script_path.parent.parent.parent # 根據 apps/podcasts/ 結構回推根目錄
    ...
    for i, entry in enumerate(selected):
        ...
        try:
            ...
            audio_path = show_dir / f"{safe_title}{os.path.splitext(url.split('?')[0])[1] or '.audio'}"
