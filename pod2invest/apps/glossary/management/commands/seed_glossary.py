...
def upsert_term_orm(
    ...
    obj, _created = GlossaryTerm.objects.update_or_create(
        term=term,
        defaults={
            "category": category,
    ...
    for a in aliases:
        ...
        if not a:
            continue
        GlossaryAlias.objects.get_or_create(term=obj, alias=a)

    return obj


class Command(BaseCommand):
    help = "Seed glossary terms into Django DB (ORM)."

    def add_arguments(self, parser):
        parser.add_argument(
            "--reset",
            action="store_true",
            help="Delete existing glossary terms & aliases before seeding.",
        )

    @transaction.atomic
    def handle(self, *args, **options):
        if options["reset"]:
            GlossaryAlias.objects.all().delete()
            GlossaryTerm.objects.all().delete()
            self.stdout.write(self.style.WARNING("Reset glossary tables."))

        # ✅ 把你原本 seed_terms.py 內的 upsert_term(...) 全部搬到這裡，
        #    但把 upsert_term 換成 upsert_term_orm
        ...
        upsert_term_orm(
            ...
            long_definition=(
                "創業投資，指由一群具有技術、財務、市場或產業專業知識和經驗的人士操作，"
        ...
        upsert_term_orm(
            ...
            long_definition=(
                "藉由投資不同類型、關聯性低的資產（股票、債券、外幣、期權、貴金屬、金融衍生工具、房地產、土地、"
        ...
        upsert_term_orm(
            ...
            long_definition=(
                "價值股就是價值（公司未來獲利能力）相對價格被低估的股票。一般會用股價相對每股盈餘、淨值及現金股利的比例來衡量，"
        ...
        upsert_term_orm(
            term="淨值",
            aliases=["ETF淨值", "每單位淨值", "NAV", "Net Asset Value"],
            category="ETF與指數",
            short_definition="ETF的真實價值（NAV）：資產總值扣除負債後，再除以總單位數。",
            long_definition=("ETF的真實價值，計算方式是將持有的所有股票、債券等資產總值，扣除負債後，除以發行總單位數。"
                             "例如淨值100元代表每單位ETF實際價值100元。"),
            lang="zh-TW",
        )
        ...
        upsert_term_orm(
            term="市價",
            aliases=["Market Price", "成交市價"],
            category="ETF與指數",
            short_definition="ETF在證券市場實際買賣成交的價格；可能高於或低於淨值。",
            long_definition=("投資人在證券市場買賣ETF的實際成交價格。市價可能高於或低於淨值，形成溢價或折價。"),
            lang="zh-TW",
        )
        ...
        upsert_term_orm(
            ...
            category="ETF報酬與配息",
            short_definition="把配息用來再買進ETF單位，以放大複利與長期總報酬。",
            long_definition=("將配息自動轉購ETF單位。例如領到5,000元股息，以市價100元買進50股，長期可放大複利效果。"),
            lang="zh-TW",
        )
        # 26. 股票型ETF
        upsert_term_orm(
